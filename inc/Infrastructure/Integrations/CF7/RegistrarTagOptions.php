...
final class RegistrarTagOptions
{
    ...
    private function __construct()
    {
        ...
        add_filter('wpcf7_form_tag_data_option', [$this, 'tagTourNameList'], 10, 3);
    ...
    public function tagSelectTourType($data, $options, $args)
    {
        ...
        $term_id_names = $this->tour_repo->getTourTypeTermNames();


        // Check empty
        if (empty($term_id_names)) {
            return [];
        }

        // CF7 yêu cầu mảng giá trị đơn giản (không key => value nếu không cần)
        return array_values($term_id_names);
    }
    ...
    public function tagTourNameList($data, $options, $args) {
        if (!in_array('tour-post-type-name-data', $options)) {
            return $data;
        }

        return $this->tour_repo->getAllNames();
    }
